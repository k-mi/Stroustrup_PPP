...
namespace Graph_lib{
	...
	istream& operator>>(istream& is, Collection& c){
		...
		if(is >> ch1 >> cc.length >> ch2 >> cc.count >> ch3){
			if(ch1 != '(' || ch2 != ',' || ch3 != ')'){
...
int main(){
	const string file_name = "q15_8_input.txt";
	ifstream ifs(file_name.c_str());
	if(!ifs) error("can't open ", file_name);

	Simple_window win(Point(100, 100), 600, 400, "Height");

	Line_chart lc(Point(0, 0), win.x_max(), win.y_max());

	Collection c;
	while(ifs >> c){
		if(c.length < 0 || c.count < 0) error("out of range");
		
		lc.add_data(c.count, to_string(int(c.length)));
