/*
	Simple calculator (q7_8.cpp) + Bitwise operation
...
double primary(){
	...
	switch(t.kind){
		...
		{
			double p = primary();
			if(p < 0) error("operand is non-positive value");
			unsigned int i1 = static_cast<unsigned int>(p);
			if(i1 != p) error("operand is not unsigned int");
			return ~i1;
			//t = ts.get();
		}
		case '-':
	...
	while(true){
		switch(t.kind){
			...
			case '^':
				{
					double d = primary();
					if(left < 0|| d < 0) error("operand is non-positive value");
					unsigned int i1 = static_cast<unsigned int>(left);
					if(i1 != left) error("left-hand operand is not unsigned int");
					unsigned int i2 = static_cast<unsigned int>(d);
					if(i2 != d) error("right-hand operand is not unsigned int");
					
					// ビット演算
					if(t.kind == '&') left = i1 & i2;
					if(t.kind == '|') left = i1 | i2;
					if(t.kind == '^') left = i1 ^ i2;
...
void calculate(){
	while (cin){
		try{
			...
			ts.putback(t);
	    	cout << result << setprecision(15) << statement() << endl;		    		
		}
		catch(exception& e){
			cerr << e.what() << endl;
			clean_up_mess();
