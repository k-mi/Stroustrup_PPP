...
#ifdef OBJ_TEXT
	cout << "Line(Point(" << p1.x << "," << p1.y << "),Point(" << p2.x << "," << p2.y << "))" << endl;
#endif
}
...
bool line_segment_intersect(Point p1, Point p2, Point p3, Point p4, Point& intersection){
   ...
   intersection.x = (int)(p1.x + u.first*(p2.x - p1.x));
   intersection.y = (int)(p1.y + u.first*(p2.y - p1.y));
   return true;
}
...
Suffix::Encoding get_encoding(const string& s)
{
    ...
    for (int i = 0, n = ARRAY_SIZE(smap); i < n; i++)
    {
        ...
        if (s.length() >= (unsigned)len && s.substr(s.length()-len, len) == smap[i].extension)
