/*
	...
	q8-1
	main関数内のToken_streamのコンストラクタで入力元となるistream&を指定できるように変更。	
*/
...
class Token_stream{
public:
	Token_stream();				// cinから読み取るToken_streamを作成する
	Token_stream(istream& is);	// istreamから読み取るToken_streamを作成する
...
Token_stream::Token_stream() : full(false), buffer(0), input(cin) {}
Token_stream::Token_stream(istream& is) : full(false), buffer(0), input(is) {}
...
Token Token_stream::get(){
	...
	char ch = input.get();		// 1文字取得（ホワイトスペースを読み飛ばさない）
	while(isspace(ch)){
	...
	switch(ch){
	    ...
	    case '5': case '6': case '7': case '8': case '9':	// 数値リテラル
	        {    
	            input.putback(ch);         	// 数字を入力ストリームに戻す
	            int val;
	            input >> val;              	// 整数を読み取る
...
double expression(Token_stream&);	// primary関数からexpression関数を呼び出せるようにする宣言
...
double expression(Token_stream& ts){
	double left = term(ts);	// Termを読み取って評価する
	Token t = ts.get();		// 次のトークンを取得する

	while(true){	// +または-を探す
		switch(t.kind){
			case '+':
				left += term(ts);		// Termを評価して足す
				t = ts.get();
				break;
			case '-':
				left -= term(ts);		// Termを評価して引く
				t = ts.get();
				break;
			default:
				ts.putback(t);
...
double statement(Token_stream& ts){
	...
	switch(t.kind){
		...
		case constt:
			ts.putback(t);
			return declaration(ts);	// 変数・定数の宣言
		default:
			ts.putback(t);
...
void calculate(Token_stream& ts){
	while (cin){
		try{
			...
			if(d > numeric_limits<int>::max()) error("overflow");
			if(d < numeric_limits<int>::min()) error("underflow");
	    	cout << result << int(d) << endl;		    		
		}
		catch(exception& e){
			cerr << e.what() << endl;
			clean_up_mess(ts);
