...
int main(){
	...
	// 2つのファイルを3つ目のファイルとしてマージ
	vector<Order> v1, v2;
	file_input(v1, "q21_9_output1.txt");
	sort(v1.begin(), v1.end(), Sort_by_name());
	file_input(v2, "q21_9_output2.txt");
	sort(v2.begin(), v2.end(), Sort_by_name());

	vector<Order> v3(v1.size() + v2.size());
	merge(v1.begin(), v1.end(), v2.begin(), v2.end(), v3.begin(), Sort_by_name());
	file_output(v3.begin(), v3.end(), "q21_9_output3.txt");
}