...
void make(vector<Room>& rooms){
	for(int i = 0; i < rooms.size(); ++i){
		rooms[i].adjacent1 = i - 1 >= 0 ? i - 1 : rooms.size() - 1;
		rooms[i].adjacent2 = i + 1 < rooms.size() ? i + 1 : 0;
		rooms[i].adjacent3 = i + rooms.size() / 2 < rooms.size() ? 
							 i + rooms.size() / 2 : i - rooms.size() / 2;	// room_max/2を加えた要素と接続
	}
...
int game(int room_max = 20){
	vector<Room> rooms(room_max);		// 部屋リスト
	make(rooms);						// 洞窟を作成
...
int main(){
	cout << "*** Hunt the Wumpus ***" << endl;	// 開始時のメッセージ

	srand(time(NULL));
	int room_max = 20;	// 部屋数（4以上の偶数）

	while(cin.good()){
		// 初期化
		cout << "input number of rooms (up to 4 & even number) "; 
		cin >> room_max;
		if(room_max < 4 || room_max % 2 != 0)
			room_max = 8;
